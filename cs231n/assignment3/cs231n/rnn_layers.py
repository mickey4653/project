...
def rnn_step_backward(dnext_h, cache):
    ...
    # Gradients of previous hidden state
    dprev_h = np.dot(dh_raw, Wh.T)
    # Gradients of input-to-hidden weights
    dWx = np.dot(x.T, dh_raw)
    # Gradients of hidden-to-hidden weights
    dWh = np.dot(prev_h.T, dh_raw)
    #  Gradients of bias vector
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # get shape
    N, T, D = x.shape
    #  The RNN uses a hidden size of H
    H = Wh.shape[0]
...
def rnn_backward(dh, cache):
    ...
    dprev_h = 0
    # Implement the backward pass for a vanilla RNN running an entire sequence of data
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros_like(W)
    # return gradient for the word embedding matrix.
    np.add.at(dW, x, dout)
